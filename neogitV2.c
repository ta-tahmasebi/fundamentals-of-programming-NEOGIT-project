...
//end of merge
//revert
void revert(int id, char* massage, char mode){ //mode m -> new massage
    check_near_main();
    char* addrress_info = absolute_address_neogit();
    addrress_info = connectTwoString(addrress_info, "\\commits\\");
    char temp [20];
    sprintf(temp, "%d_info", id);
    addrress_info = connectTwoString(addrress_info, temp);
    //printf("%s", addrress_info);
    char exist = check_type(addrress_info);
    if(exist != 1){
        printf("\033[31mInvalid\033[0m commit id! \n");
        exit(0);
    }
    sprintf(temp, "%d", id);
    int current = get_last_hash('c');
    char* branch = get_current_brach();
    folder info = extract_data_from_commit(temp ,addrress_info);
    if(strcmp(info.banch, branch)){
        printf("commit is \033[31mnot\033[0m in this branch!\n");
        exit(0);
    }
    //printf("%s", info.massage);
    checkOut_byID(id);
    clear_stage();
    system("neogit add . > nul");
    printf("now you are on \033[32mnew commit\033[0m. wait for commiting ...\n");
    if(mode != 'm') massage = info.massage;
    commit(massage);
}
void revert_n(int id){
    check_near_main();
    char* addrress_info = absolute_address_neogit();
    addrress_info = connectTwoString(addrress_info, "\\commits\\");
    char temp [20];
    ...
    sprintf(temp, "%d", id);
    checkOut_byID(id);
    printf("now you are on \033[32mnew area\033[0m.\n");
}
...
void get_commands_V2(char**  input, int len){
    ...
    //revert
    if(equalStrings(input[1], "revert") && strstr(input[2], "HEAD-") == input[2] && len == 3){
        parametrs condition;
        int number = 0;
        sscanf(input[2], "HEAD-%d", &number); 
        if(number <= 0){
            printf("\033[31mInvalid number\033[0m!\n");
            exit(0);
        }
        ...
        for(int i = 0; list[i] != NULL; i++){
            if(i == number){
                char command[300];
                sprintf(command, "neogit revert %s", list[i]->name);
                system(command);
            }
        }
        exit(0);
    }
    if(equalStrings(input[1], "revert") && equalStrings(input[2], "-m") && strcmp(input[3], "") && strstr(input[4], "HEAD-") == input[4] && len == 5){
        parametrs condition;
        int number = 0;
        sscanf(input[4], "HEAD-%d", &number);
        if(number <= 0){
            printf("\033[31mInvalid number\033[0m!\n");
            exit(0);
        }
        ...
        for(int i = 0; list[i] != NULL; i++){
            if(i == number){
                char command[300];
                sprintf(command, "neogit revert -m \"%s\" %s", input[3] , list[i]->name);
                system(command);
            }
        }
        exit(0);
    ...
    if(equalStrings(input[1], "revert") && strcmp(input[2], "") && len == 3){
        revert(atoi(input[2]), "", 'n');
        exit(0);
    }
    if(equalStrings(input[1], "revert") && equalStrings(input[2], "-m") && strcmp(input[3], "") && strcmp(input[4], "") && len == 5){
        revert(atoi(input[4]), input[3], 'm');
        exit(0);
    }
    if(equalStrings(input[1], "revert") && equalStrings(input[2], "-n") && strcmp(input[3], "") && len == 4){
        revert_n(atoi(input[3]));
        exit(0);
    }
